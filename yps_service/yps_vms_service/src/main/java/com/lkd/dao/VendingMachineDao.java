...
public interface VendingMachineDao extends BaseMapper<VendingMachineEntity> {
    @Results(id = "vmMapper",value = {
            ...
            @Result(property = "type",column = "vm_type",one = @One(select = "com.yps.dao.VmTypeDao.selectById")),
            @Result(property = "node",column = "node_id",one = @One(select = "com.yps.dao.NodeDao.getById")),
